...
void execute_parallel_forward(const torch::PackedTensorAccessor32<scalar_t, 3, torch::RestrictPtrTraits>& mixture_a,
    ...
    for (uint i = 0; i < n.batch * n.layers * n.xes; ++i) {
        ...
        for (uint c = 0; c < n.components; ++c) {
            const auto& c_weight = gm::weight(mixture_a[batch_layer_index][int(c)]);
            const auto& c_pos = gm::position<DIMS>(mixture_a[batch_layer_index][int(c)]);
            const auto& c_cov = gm::covariance<DIMS>(mixture_a[batch_layer_index][int(c)]);
