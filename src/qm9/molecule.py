...
class Molecule:
    ...
    def as_gaussian_mixture(self, atom_weights: torch.Tensor, atom_radii: torch.Tensor) -> torch.Tensor:
        device = atom_weights.device
        weights = torch.zeros(1, len(Molecule.ATOM_TYPES) * 2, Molecule.N_ATOMS_MAX_PER_TYPE, 1, device=device)
        positions = torch.zeros(1, len(Molecule.ATOM_TYPES) * 2, Molecule.N_ATOMS_MAX_PER_TYPE, 3, device=device)
        radii = torch.ones(1, len(Molecule.ATOM_TYPES) * 2, Molecule.N_ATOMS_MAX_PER_TYPE, 1, device=device)
