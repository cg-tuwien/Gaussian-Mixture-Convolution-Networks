...
cuda_extra_cuda_cflags = ["-O3",  "--use_fast_math", "--expt-extended-lambda", "--std=c++17", " --expt-relaxed-constexpr", "-DNDEBUG"]  # , "-DNDEBUG"
if platform.system() == "Windows":
    cuda_extra_cflags = ["/openmp", "/O2", "/fp:fast", "/DGPE_NO_CUDA_ERROR_CHECKING", "/DNDEBUG"]
    cpp_extra_cflags = ["/openmp", "/O2", "/fp:fast", "/std:c++17", "/DNDEBUG"]
    ...
    cpp_extra_cflags = ["-fopenmp", "-ffast-math", " -fno-finite-math-only", "-O4", "-march=native", "--std=c++17", "-DGPE_NO_CUDA_ERROR_CHECKING", "-DNDEBUG"]  # , "-DNDEBUG"
