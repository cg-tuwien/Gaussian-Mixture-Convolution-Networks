...
struct Tree {
    ...
    Tree(const torch::Tensor& data, const torch::Tensor& kernels, const Config& config) : m_data(data), m_kernels(kernels), m_config(config) {
        n = gpe::get_ns(data);
    ...
    torch::Tensor compute_morton_codes(const torch::Tensor& data, const torch::Tensor& kernels) const {
        ...
        auto aabb_a = gpe::accessor<float, 4>(aabbs);
