...
fitconf = pcfit.Config(n_gaussians=32, eps=0.00001, gengmm_path=tmp_gmm_base_path)
# pcfit.fit(fitconf)
...
c.model.layers = [Layer(8, 2.5, 16),
                  Layer(16, 2.5, 8),
                  Layer(32, 2.5, 4),
                  Layer(10, 2.5, -1)]
main.experiment(device=device, desc_string=f"{fitconf.name}_{c.produce_description()}", config=c, ablation_name="n_gaussians")
