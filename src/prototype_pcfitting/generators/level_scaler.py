...
class LevelScaler:
    ...
    def set_pointcloud(self, pcbatch: torch.Tensor, parent_per_point: torch.Tensor, relevant_parents: torch.Tensor):
        # Has to be called before scaling!
        # This extracts the scalings from the pointcloud (1, n, 3)
        # parent_per_point: (1, n)
        # relevant_parents: (1, m)
        n = parent_per_point.shape[1]
        m = relevant_parents.shape[1]

        self._m = m
        self._parent_per_point = parent_per_point
        self._relevant_parents = relevant_parents

        self.scaleP = torch.zeros(m).to(pcbatch.dtype).cuda() # shape: (m)
        self.offsetP = torch.zeros(m, 3).to(pcbatch.dtype).cuda() # (m,3)
        for i in range(m):
            rel_point_mask = (parent_per_point == relevant_parents[0, i])
            rel_points = pcbatch[rel_point_mask]    # Calling function must make sure that this is more than 1 point, (s, 3)
    ...
    def scale_down_pc(self, pcbatch: torch.Tensor) -> torch.Tensor:
        ...
        scaleddown = (pcbatch[0,:,:] - self.offsetP[indizes, :]) / self.scaleP[indizes, :]
        return scaleddown
    ...
    def scale_up_gmm_wpc(self, weights: torch.Tensor, positions: torch.Tensor, covariances: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        ...
        j = int(weights.shape[2] / self._relevant_parents.shape[1])
