...
float eval_psnr(torch::Tensor pointcloudSource, torch::Tensor pointcloudGenerated)
{
    omp_set_dynamic(0);
    ...
    float relrms = bboxPointsS.diagonal() / std::sqrt(avgminsqdiff);
    relrms = 20 * std::log10(relrms);
    return relrms;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("eval_psnr", &eval_psnr);
}